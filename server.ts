...
// Fast live AI caller with multi-model fallback and reliable timeout
async function callNutritionLiveAi(systemPrompt: string, userMessage: string, timeoutMs: number = 12000): Promise<string | null> {
  ...
  // 2. Attempt Gemini with configured Google AI key (multi-model fallback prioritizing fastest flash-lite)
      ...
      for (const model of candidateModels) {
        try {
          ...
          const timeoutCall = new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error(`Gemini ${model} timeout (${timeoutMs}ms)`)), timeoutMs)
          );
          const response = await Promise.race([geminiCall, timeoutCall]);
          if (response?.text && response.text.trim().length > 0) {
            console.log(`[Nutrition AI] ✅ Google Gemini (${model}) successfully generated response for: "${userMessage}"`);
            return response.text;
          }
        } catch (mErr: any) {
          console.warn(`[Nutrition AI] Google Gemini (${model}) attempt notice:`, mErr?.message || mErr);
        }
      }
    } catch (geminiErr: any) {
      console.warn(`[Nutrition AI] Google Gemini client error (${geminiErr?.message || geminiErr}), falling back to intelligent clinical synthesizer`);
    }
  }
...
const SERVER_CATEGORY_UNSPLASH_MAP: Record<string, string> = {
  ...
  dosai: "https://images.unsplash.com/photo-1668236543090-82eba5ee5976?auto=format&fit=crop&w=800&q=80",
  idli: "https://images.unsplash.com/photo-1589301760014-d929f3979dbc?auto=format&fit=crop&w=800&q=80",
  uttapam: "https://images.unsplash.com/photo-1668236543090-82eba5ee5976?auto=format&fit=crop&w=800&q=80",
  appam: "https://images.unsplash.com/photo-1668236543090-82eba5ee5976?auto=format&fit=crop&w=800&q=80",
  upma: "https://images.unsplash.com/photo-1546833999-b9f581a1996d?auto=format&fit=crop&w=800&q=80",
  pongal: "https://images.unsplash.com/photo-1546833999-b9f581a1996d?auto=format&fit=crop&w=800&q=80",
  ...
  cheela: "https://images.unsplash.com/photo-1589301760014-d929f3979dbc?auto=format&fit=crop&w=800&q=80",
  pancake: "https://images.unsplash.com/photo-1589301760014-d929f3979dbc?auto=format&fit=crop&w=800&q=80",
  ...
  phulka: "https://images.unsplash.com/photo-1626074353765-517a681e40be?auto=format&fit=crop&w=800&q=80",
  ...
  pulav: "https://images.unsplash.com/photo-1563379091339-03b21ab4a4f8?auto=format&fit=crop&w=800&q=80",
  ...
  // Oats & Healthy Porridge
  oats: "https://images.unsplash.com/photo-1517673132405-a56a62b18caf?auto=format&fit=crop&w=800&q=80",
  porridge: "https://images.unsplash.com/photo-1517673132405-a56a62b18caf?auto=format&fit=crop&w=800&q=80",
  ...
  broth: "https://images.unsplash.com/photo-1547592166-23ac45744acd?auto=format&fit=crop&w=800&q=80",
  rasam: "https://images.unsplash.com/photo-1547592166-23ac45744acd?auto=format&fit=crop&w=800&q=80",
  sambar: "https://images.unsplash.com/photo-1546833999-b9f581a1996d?auto=format&fit=crop&w=800&q=80",

  // Dairy & Protein
  paneer: "https://images.unsplash.com/photo-1567188040759-fb8a883dc6d8?auto=format&fit=crop&w=800&q=80",
  palak: "https://images.unsplash.com/photo-1567188040759-fb8a883dc6d8?auto=format&fit=crop&w=800&q=80",
  ...
  chana: "https://images.unsplash.com/photo-1546833999-b9f581a1996d?auto=format&fit=crop&w=800&q=80",
  sundal: "https://images.unsplash.com/photo-1546833999-b9f581a1996d?auto=format&fit=crop&w=800&q=80",

  // Non-Veg (Clean Cooked)
  ...
  sprouts: "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?auto=format&fit=crop&w=800&q=80",
  ...
  makhana: "https://images.unsplash.com/photo-1599599810769-bcde5a160d32?auto=format&fit=crop&w=800&q=80",
  ...
  juice: "https://images.unsplash.com/photo-1553530666-ba11a7da3888?auto=format&fit=crop&w=800&q=80",

  // Sweets & Warm Drinks
  kheer: "https://images.unsplash.com/photo-1599490659213-e2b9527bd087?auto=format&fit=crop&w=800&q=80",
  payasam: "https://images.unsplash.com/photo-1599490659213-e2b9527bd087?auto=format&fit=crop&w=800&q=80",
  halwa: "https://images.unsplash.com/photo-1599490659213-e2b9527bd087?auto=format&fit=crop&w=800&q=80",
  chutney: "https://images.unsplash.com/photo-1626777552726-4a6b54c97e46?auto=format&fit=crop&w=800&q=80",
  milk: "https://images.unsplash.com/photo-1576092768241-dec231879fc3?auto=format&fit=crop&w=800&q=80",
...
function getCategoryUnsplashFallback(dishName: string): string {
  ...
  for (const [key, url] of Object.entries(SERVER_CATEGORY_UNSPLASH_MAP)) {
    if (d.includes(key)) return url;
  }
  if (d.includes("sweet") || d.includes("laddu") || d.includes("ladoo") || d.includes("dessert")) {
    return SERVER_CATEGORY_UNSPLASH_MAP.kheer;
  }
  if (d.includes("gravy") || d.includes("masala") || d.includes("kootu") || d.includes("kurma") || d.includes("kozhambu") || d.includes("kuzhambu")) {
    return SERVER_CATEGORY_UNSPLASH_MAP.curry;
  }
  if (d.includes("tiffin") || d.includes("snack") || d.includes("tikki") || d.includes("cutlet")) {
    return SERVER_CATEGORY_UNSPLASH_MAP.chilla;
  }
...
4. Output raw JSON only with NO markdown fences.`;
  ...
  try {
    const rawAiText = await callNutritionLiveAi(systemPrompt, query, 12000);
    if (rawAiText) {
...
2. Output raw JSON only with NO markdown fences.`;
  ...
  try {
    const rawAiText = await callNutritionLiveAi(systemPrompt, dishName, 12000);
    if (rawAiText) {
      const cleaned = rawAiText.replace(/```json/gi, "").replace(/```/g, "").trim();
      const parsed = JSON.parse(cleaned);
...
function generateOfflineRecipeResult(dishName: string, trimester: number = 2) {
  ...
  // 3. IDLI (Steamed South Indian Breakfast)
  if (d.includes("idli") || d.includes("steamed cake")) {
    return {
      dishName: `Maternal Gut-Friendly Steamed ${cleanTitle}`,
      category: "Steamed Breakfast / Easy Digestion",
      prepTime: "10 mins",
      cookTime: "12 mins",
      servings: 2,
      imageUrl: SERVER_CATEGORY_UNSPLASH_MAP.idli,
      safetyStatus: "SAFE",
      safetyMessage: `🟢 Excellent in Pregnancy: Naturally fermented and 100% oil-free steamed ${cleanTitle} provides gut-friendly probiotics, bioavailable B-vitamins, and effortless digestibility without triggering heartburn.`,
      nutrition: {
        calories: 190,
        protein_g: 8.5,
        proteinG: 8.5,
      ...
      ingredients: [
        { name: "Fermented Urad Dal & Rice Batter", quantity: "2 cups", notes: "Naturally fermented overnight" },
        { name: "Finely Grated Carrots or Beetroot", quantity: "1/2 cup", notes: "Washed & folded into batter for iron & beta-carotene" },
        { name: "A2 Cow Ghee", quantity: "1/2 tsp", notes: "For lightly greasing steamer plates" },
        { name: "Mild Vegetable & Drumstick Sambar", quantity: "1 cup", notes: "Rich in calcium & vitamin C" },
        { name: "Fresh Mint-Coconut Chutney", quantity: "1/4 cup", notes: "Freshly ground probiotic condiment" }
      ],
      instructions: [
        "1. Whisk the naturally fermented batter gently; fold in grated carrots or beetroot for extra iron and vitamins.",
        "2. Lightly grease the idli steamer moulds with a drop of cow ghee.",
        "3. Spoon the batter into each mould cavity until 3/4 full to allow rising.",
        "4. Steam in a covered steamer or pressure cooker (without weight whistle) for 10–12 minutes on medium flame until a toothpick comes out clean.",
        "5. Let rest for 2 minutes, unmould with a wet spoon, and serve steaming hot with warm drumstick sambar and fresh coconut chutney."
      ],
      pregnancyBenefits: [
        "Fermentation increases bioavailability of Folate (Vitamin B9) and gut-friendly probiotics",
        "Steaming requires zero frying, completely eliminating heartburn and late-stage acid reflux",
        "Urad dal provides essential plant amino acids for embryonic cell construction"
      ],
      additionalInfo: [
        "Ensure batter is fresh and not sour; always consume freshly steamed idlis."
      ],
      trimesterGuidance: "Highly recommended for Trimester 1 (nausea relief) and Trimester 3 (gentle on compressed stomach).",
      sourceType: "AI_GENERATED",
      confidence: "HIGH"
    };
  }

  // 4. DOSA / UTTAPAM / APPAM (Crisp / Soft Tawa Tiffin)
  if (d.includes("dosa") || d.includes("dosai") || d.includes("uttapam") || d.includes("appam")) {
    return {
      dishName: `Maternal Wholesome ${cleanTitle}`,
      category: "Breakfast / Light Meal",
      prepTime: "10 mins",
      cookTime: "12 mins",
      servings: 2,
      imageUrl: SERVER_CATEGORY_UNSPLASH_MAP.dosa,
      safetyStatus: "SAFE",
      safetyMessage: `🟢 Good for Pregnancy: Fermented rice & lentil ${cleanTitle} provides sustained complex carbs, low glycemic index, and maternal stamina. Cook with pure cow ghee.`,
      nutrition: {
        calories: 260,
        protein_g: 9,
        proteinG: 9,
      ...
      ingredients: [
        { name: "Fermented Dosa / Millet Batter", quantity: "2 cups", notes: "Freshly fermented" },
        { name: "Finely Chopped Spinach or Fenugreek Leaves", quantity: "1/2 cup", notes: "Washed & stirred into batter" },
        { name: "A2 Cow Ghee", quantity: "1 tbsp", notes: "For pan crisping" },
        { name: "Cumin Seeds (Jeera)", quantity: "1/2 tsp", notes: "Aids gastric digestion" },
        { name: "Fresh Mint-Coconut Chutney", quantity: "1/4 cup", notes: "Fresh condiment" }
      ],
      instructions: [
        "1. Whisk batter to spreading consistency; mix in cumin seeds and finely chopped greens.",
        "2. Heat a cast-iron tawa over medium flame. Smear 1/2 tsp cow ghee across the surface.",
        "3. Pour a ladleful of batter in the center, gently swirling outwards in concentric circles.",
        "4. Drizzle a little ghee along the edges and cook on medium flame for 2-3 minutes until golden crisp.",
        "5. Flip gently if making soft dosa or fold in half if making crisp roast; serve warm with chutney and sambar."
      ],
      pregnancyBenefits: [
        "Fermented batter releases bioavailable minerals and keeps digestion smooth",
        "Moderate glycemic index sustains maternal energy without blood sugar spikes",
        "Cow ghee provides fat-soluble vitamins (A, D, E, K)"
      ],
      additionalInfo: [
        "Cook in cast-iron cookware to naturally fortify with dietary trace iron."
      ],
      trimesterGuidance: "Great for Trimester 2 & 3 when energy and iron demands increase.",
      sourceType: "AI_GENERATED",
      confidence: "HIGH"
    };
  }

  // 5. POHA (Flattened Rice Superfood)
  if (d.includes("poha") || d.includes("aval") || d.includes("avalki")) {
    return {
      dishName: `Iron-Rich Maternal ${cleanTitle}`,
      category: "Quick Nutrient Breakfast",
      prepTime: "8 mins",
      cookTime: "10 mins",
      servings: 2,
      imageUrl: SERVER_CATEGORY_UNSPLASH_MAP.poha,
      safetyStatus: "SAFE",
      safetyMessage: `🟢 Good for Pregnancy: Flattened rice (poha) is a traditional Indian superfood dense in non-heme iron. Paired with lemon juice for 300% greater iron absorption.`,
      nutrition: {
        calories: 250,
        protein_g: 8.5,
        proteinG: 8.5,
      ...
      ingredients: [
        { name: "Thick Flattened Rice (Poha / Aval)", quantity: "1.5 cups", notes: "Rinsed & drained" },
        { name: "Roasted Peanuts", quantity: "2 tbsp", notes: "Rich in folate & biotin" },
        { name: "Boiled Potato Cubes & Green Peas", quantity: "1/2 cup", notes: "Cleaned & steamed" },
        { name: "Mustard Seeds, Cumin & Curry Leaves", quantity: "1 tsp", notes: "Tempering herbs" },
        { name: "Turmeric Powder & Rock Salt", quantity: "1/2 tsp", notes: "Anti-inflammatory" },
        { name: "Cow Ghee", quantity: "1 tbsp", notes: "For cooking" },
        { name: "Fresh Lemon Juice & Coriander", quantity: "1 tbsp", notes: "Vitamin C booster" }
      ],
      instructions: [
        "1. Place thick poha in a colander, rinse gently under running water for 30 seconds, and drain completely. Let rest for 5 minutes so flakes soften.",
        "2. Heat ghee in a skillet, add mustard seeds, cumin seeds, and fresh curry leaves until they crackle.",
        "3. Add chopped onions, turmeric, steamed potatoes, and green peas. Sauté for 3 minutes.",
        "4. Gently fold in the softened poha and salt. Cover with a lid and steam on very low flame for 2-3 minutes.",
        "5. Turn off heat, stir in crunchy roasted peanuts, squeeze fresh lemon juice, and garnish with fresh coriander before serving warm."
      ],
      pregnancyBenefits: [
        "Flattened rice provides high non-heme iron to counter maternal anemia",
        "Fresh lemon juice supplies Vitamin C, multiplying iron absorption by up to 300%",
        "Peanuts add plant protein and essential folate for fetal neural development"
      ],
      additionalInfo: [
        "Use thick poha rather than thin nylon poha to prevent sogginess during steaming."
      ],
      trimesterGuidance: "Ideal across all three trimesters as an easily digestible, iron-packed breakfast.",
      sourceType: "AI_GENERATED",
      confidence: "HIGH"
    };
  }

  // 6. UPMA & PONGAL (Comforting Fiber Bowl)
  if (d.includes("upma") || d.includes("pongal") || d.includes("kichadi")) {
    const isPongal = d.includes("pongal");
    return {
      dishName: `Wholesome Nourishing ${cleanTitle}`,
      category: "Fiber-Rich Comfort Meal",
      prepTime: "10 mins",
      cookTime: "15 mins",
      servings: 2,
      imageUrl: SERVER_CATEGORY_UNSPLASH_MAP.upma,
      safetyStatus: "SAFE",
      safetyMessage: `🟢 Good for Pregnancy: Warm ${cleanTitle} provides slow-burning complex carbs, gentle dietary fiber, and soothing spices that calm the maternal stomach.`,
      nutrition: {
        calories: 280,
        protein_g: 11,
        proteinG: 11,
      ...
      ingredients: [
        { name: isPongal ? "Raw Rice & Yellow Moong Dal (1:1 ratio)" : "Roasted Ragi / Wheat Semolina (Rava)", quantity: "1.5 cups", notes: "Washed / dry-roasted" },
        { name: "Finely Chopped Carrots, Beans & Green Peas", quantity: "1 cup", notes: "Steam chopped" },
        { name: "Cow Ghee", quantity: "1.5 tbsp", notes: "Healthy maternal fats" },
        { name: "Crushed Black Pepper & Cumin Seeds", quantity: "1 tsp", notes: "Eases digestion & gas" },
        { name: "Minced Fresh Ginger & Curry Leaves", quantity: "1 tbsp", notes: "Soothes morning nausea" },
        { name: "Whole Cashew Halves", quantity: "1 tbsp", notes: "Roasted in ghee" },
        { name: "Hot Water", quantity: "3 cups", notes: "For cooking" }
      ],
      instructions: [
        `1. ${isPongal ? "Wash rice and moong dal, pressure cook with 3.5 cups water and turmeric for 3 whistles until soft and mushy." : "Dry roast semolina/millet rava in a dry pan on low heat for 4 minutes until aromatic, then set aside."}`,
        "2. Heat cow ghee in a pan, roast cashews until golden, and remove.",
        "3. In the same ghee, add cumin seeds, crushed black peppercorns, fresh ginger, and curry leaves until fragrant.",
        `4. ${isPongal ? "Pour the hot ghee tempering over the cooked rice-dal mixture and stir well." : "Add chopped vegetables and sauté for 3 minutes. Pour in 3 cups boiling water with salt. Slowly drizzle in roasted rava while stirring continuously to avoid lumps."}`,
        "5. Cover with lid, simmer on low heat for 4-5 minutes until cooked soft and fluffy.",
        "6. Top with roasted cashews and serve warm with coconut chutney or cooling curd."
      ],
      pregnancyBenefits: [
        "Crushed black pepper and ginger stimulate sluggish digestive enzymes and ease acidity",
        "High dietary fiber prevents late-pregnancy hemorrhoids and constipation",
        "Moong dal and cashews deliver essential amino acids for fetal tissue construction"
      ],
      additionalInfo: [
        "Always consume warm; reheating multiple times depletes natural flavors."
      ],
      trimesterGuidance: "Wonderful comfort food throughout Trimester 1, 2, and 3.",
      sourceType: "AI_GENERATED",
      confidence: "HIGH"
    };
  }

  // 7. OATS & HEALTHY PORRIDGE (Beta-Glucan Fiber Bowl)
  if (d.includes("oats") || d.includes("porridge") || d.includes("muesli") || d.includes("dalia") || d.includes("sevai") || d.includes("vermicelli")) {
    return {
      dishName: `Heart-Healthy Maternal ${cleanTitle}`,
      category: "Fiber & Micronutrient Bowl",
      prepTime: "5 mins",
      cookTime: "10 mins",
      servings: 1,
      imageUrl: SERVER_CATEGORY_UNSPLASH_MAP.oats,
      safetyStatus: "SAFE",
      safetyMessage: `🟢 Good for Pregnancy: Whole rolled oats and grains provide beta-glucan soluble fiber, stabilizing maternal blood glucose and keeping morning sickness away.`,
      nutrition: {
        calories: 290,
        protein_g: 12,
        proteinG: 12,
      ...
      ingredients: [
        { name: "100% Whole Rolled Oats / Broken Wheat", quantity: "1/2 cup", notes: "High beta-glucan fiber" },
        { name: "Pasteurized Milk or Unsweetened Almond Milk", quantity: "1 cup", notes: "Thoroughly boiled" },
        { name: "Soaked Chia Seeds or Flaxseeds", quantity: "1 tbsp", notes: "Omega-3 plant ALA" },
        { name: "Sliced Ripe Banana or Blueberries", quantity: "1/2 cup", notes: "Natural sweetness & potassium" },
        { name: "Crushed Walnuts & Almonds", quantity: "1 tbsp", notes: "Fetal brain DHA precursors" },
        { name: "A Pinch of Cinnamon Powder", quantity: "1/4 tsp", notes: "Balances blood sugar" }
      ],
      instructions: [
        "1. In a small saucepan, bring milk (or water) to a gentle simmer over medium heat.",
        "2. Stir in rolled oats, pinch of salt, and cinnamon powder.",
        "3. Cook on low heat for 5-7 minutes, stirring occasionally until thick and creamy.",
        "4. Remove from heat and pour into a warm bowl.",
        "5. Stir in soaked chia seeds, and top with sliced ripe bananas, fresh berries, and crushed walnuts.",
        "6. Drizzle 1/2 tsp raw honey or pure maple syrup if desired and enjoy warm."
      ],
      pregnancyBenefits: [
        "Soluble beta-glucan fiber creates a protective gel in stomach, relieving heartburn and constipation",
        "Walnuts and chia seeds deliver plant Omega-3 fatty acids for fetal visual and cognitive foundation",
        "Low glycemic response prevents gestational diabetes blood sugar surges"
      ],
      additionalInfo: [
        "Choose rolled or steel-cut oats over instant sugary flavored oat packets."
      ],
      trimesterGuidance: "Superb daily breakfast across all trimesters.",
      sourceType: "AI_GENERATED",
      confidence: "HIGH"
    };
  }

  // 8. CHILLA / CHEELA / PANCAKE (Oats Chilla, Besan Chilla, Moong Dal Chilla)
  if (d.includes("chilla") || d.includes("cheela") || d.includes("pancake") || d.includes("crepe")) {
    ...
    return {
      ...
      cookTime: "10 mins",
      servings: 2,
      imageUrl: SERVER_CATEGORY_UNSPLASH_MAP.chilla,
      safetyStatus: "SAFE",
